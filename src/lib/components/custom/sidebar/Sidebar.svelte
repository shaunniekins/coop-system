<script lang="ts">
	...
	onMount(() => {
		function handleResize() {
			isOpen = window.innerWidth >= 640; // Adjust the breakpoint as needed
		}
...
<nav
	class="bg-gray-200 text-white w-64 flex-shrink-0 py-4 px-3 h-screen z-50"
	class:bg-opacity-50={isOpen}
	class:hidden={!isOpen}
>
	<a href="https://shaunniel.vercel.app/" class="flex items-center p-2 mb-5">
		<img
			src="https://flowbite.com/docs/images/logo.svg"
			class="h-6 mr-3 sm:h-7"
			alt="Flowbite Logo"
		/>
		<span class="self-center text-xl font-semibold whitespace-nowrap text-black">Business Name</span
		>
	</a>
	<ul class="space-y-2 font-medium">
		...
		<li>
			<a
				href="#"
				class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"
				on:click={() => setActiveSection('inventory')}
			>
				<svg
...
<button
	class="fixed bottom-4 right-4 p-2 bg-gray-800 text-white rounded-lg"
	class:hidden={isOpen}
